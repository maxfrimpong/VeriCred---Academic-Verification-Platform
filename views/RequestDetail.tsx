...
const RequestDetail: React.FC<RequestDetailProps> = ({ request, navigate, user, onUpdateRequest }) => {
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      ...
      doc.setFontSize(8);
      doc.setFont('helvetica', 'normal');
      doc.text("Generated by VeriCred Platform - Confidential Document", pageWidth / 2, pageHeight - 6, { align: 'center' });

      // Save
      doc.save(`VeriCred_Report_${request.id}.pdf`);
    } catch (e) {
